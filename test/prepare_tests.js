	...
	server.respondWith(function(request) {
		...
		switch (pathname) {
			...
			case '/jc':
				...
				break;
			case '/jl': // Handle dynamic URLs with start and end query parameters
				...
				break;
			case '/sh':
				var resp = {progTypes: [{ type: 1, name: "Type 1" }, { type: 2, name: "Type 2" }]};
				request.respond(200, resp);
				console.log("*** case sh response", resp);
				break;
			default:
				console.warn("server.respondWith error 404 for pathname: " + pathname, {url: request.url});
...
before(function() {
	// Define basic session vars
	OSApp.currentSession = {
		// very important: define prefix, ip and token
		prefix: '',
		ip: '',
		token: '',
		controller: {
