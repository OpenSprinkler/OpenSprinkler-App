...
- The source-defined production Firebase path serves Grunt's legacy `build/firmware` output; the production target is selected only by a manual workflow input (`OpenSprinkler-App/firebase.json:2-5`; `OpenSprinkler-App/Gruntfile.js:54-76`; `OpenSprinkler-App/.github/workflows/firebase-hosting.yml:84-100`).
- The Vite app emits `dist/home.js` plus `dist/assets/*`, while its workflow is manual, preview-only, and expiring (`OpenSprinkler-App/app/vite.config.ts:3-19`; `OpenSprinkler-App/.github/workflows/firebase-hosting-next.yml:1-10`; `OpenSprinkler-App/.github/workflows/firebase-hosting-next.yml:39-55`). Existing deploy instructions incorrectly point `jsp` at a `/js` subpath even though `home.js` is emitted at the site root (`OpenSprinkler-App/docs/DEPLOY.md:50-63`).
...
- App CI still runs Grunt and Karma only; it does not run the modern Vitest suite, typecheck, Vite build, or server tests (`OpenSprinkler-App/.github/workflows/test.yml:27-34`; `OpenSprinkler-App/package.json:22-35`). The 2026-07-16 design handoff records successful App/demo Vite builds, 159 Vitest checks, and a 3,319-byte gzipped production CSS baseline; CI must reproduce those results before treating them as a release gate (`OpenSprinkler-App/docs/DESIGN-HANDOFF.md:52-62`).
...
- Retire the Transifex pipeline with legacy unless a non-English language is explicitly required. Its current extraction/bundle paths are legacy-only, while modern strings are English literals (`OpenSprinkler-App/Gruntfile.js:39-42`; `OpenSprinkler-App/transifex.yml:1-7`; `OpenSprinkler-App/www/src/api/diagnostics.ts:1-5`).
...
1. **Make the evidence safe and accurate.** Remove live credentials/personal controller details from tracked runbooks, rotate exposed credentials, reconcile the drift table above, declare the `2214 + kars85` floor, and record the exact OTF revision to be tested. The current runbook contains live-controller details and a credential in tracked prose (`OpenSprinkler-App/docs/HARDWARE-VERIFICATION.md:27-46`; `OpenSprinkler-App/docs/HARDWARE-VERIFICATION.md:110-125`).
...
- Delete `www/js/modules/`, legacy Grunt/Karma paths, OSPi App branches, frozen Cordova workflows/dependencies, and Transifex/locales in one retirement change after the observation window. Do not spend a phase simplifying them first.
